...
import kopo.poly.model.RoleType;
...
public class UserService {
	...
	public class MyAuthentication extends Authenticator {
		...
		public MyAuthentication(){
			String id = "kyj2991223";
			String pw = "kkk22446688@@!";
...
//		AWS 배포시 이걸로 수정
				String host1 = "http://ec2-15-165-156-20.ap-northeast-2.compute.amazonaws.com:8000/auth/PW_FIND2";
				...
				try {
					...
					from = new InternetAddress("kyj2991223@naver.com");
					msg.setFrom(from);
					...
					msg.setText("다음 링크에 접속하여 비밀번호를 설정해주세요.인증번호 :" + Rnum +"<a href='" + host2 + "?username=" + user.getUsername() + "&email=" + user.getUSER_EMAIL() + "&Rnum=" + Rnum + "'>비밀번호 변경</a>", "UTF-8");
	...
	//	인증메일재전송
	@Transactional
	public void ReEmailsend(User user) {
		String rawPassword = user.getUSER_PASSWORD();//1234 원문
		String encPassword = encoder.encode(rawPassword);//해쉬됨


		String host1 = "http://ec2-15-165-156-20.ap-northeast-2.compute.amazonaws.com:8000/auth/";
...
//		Integer userID = null;
		...
		if (user.getUSER_OAUTH() == null) {
			...
			try {
				...
				msg.setSubject("가입 인증을 위한 이메일 인증 메일입니다", "UTF-8");
				msg.setText("다음 링크에 접속하여 이메일 인증을 진행하세요." + "<a href='" + host2 + "emailCheckAction?username=" + user.getUsername() + "&email=" + user.getUSER_EMAIL() + "&password=" + encPassword + "'>이메일 인증</a>", "UTF-8");
				msg.setHeader("content-Type", "text/html");
